...
format_disk() {
  ...
  if (( NUM_PARTS < 2 )); then
    ...
    echo "${1}p1 seems to have been created. Formatting... "
  fi
...
mount_part() {
  ...
  if [ $? -ne 0 ]; then
    ...
    green "${1} mounted on $MOUNT_POINT successfully!"
  fi
...
setup() {
  ...
  echo "export HADOOP_HOME='/home/ec2-user/hadoop'" >> .bash_profile
  echo "export HADOOP_CONF_DIR='/home/ec2-user/hadoop/etc/hadoop'" >> .bash_profile
